class VendingMachine:
    ...
    def vend(self):
        if not self.stock_num:
            ...
            if self.balance < self.price:
                ...
                change_msg = '.'
                if self.balance > 0:
                    change_msg = ' and ${0} change'.format(self.balance) + change_msg
                    self.balance = 0
                #先扣后给
                self.stock_num -= 1 
                return 'Here is your {0}'.format(self.product) + change_msg
    ...
    def restock(self, num):
        self.stock_num += num
        return 'Current {0} stock: {1}'.format(self.product, self.stock_num)
...
def store_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    # def helper(n, base_link = Link.empty):
    #     if n == 0:
    #         return base_link
    #     else:
    #         first_value = n % 10
    #         rest_n = n // 10
    #         base_link = Link(first_value, base_link)
    #         return helper(rest_n, base_link)
    # return helper(n)
    if n == 0:
        return Link(0)

    base_link = Link.empty
    while n > 0:
        first_value = n % 10
        base_link = Link(first_value, base_link)
        n = n // 10
