...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for _card in player.deck.cards[:]:
            _card.attack += other_card.attack
            _card.defense += other_card.defense
        discard_index = 0
        for _card in opponent.deck.cards[:]:
            if _card.attack == other_card.attack or _card.defense == other_card.defense:
                #discard
                opponent.deck.cards.pop(discard_index)
                discard_index -= 1
            discard_index += 1
